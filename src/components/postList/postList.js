...
const PostList = (props) => {
  ...
  const { posts, onToggleImportant, total, onChecked, onFilter, filterState } =
    props;
  ...
  const buttons = filter.map((button) => {
    return (
      <div
        ...
        key={button.name}
        className={
          button.name === filterState ? 'all-todos active' : 'all-todos'
        }>
        {button.label}
      </div>
    );
          ...
          <div className='filters'>
            <div className='slider-container'>
              <div className='slider'>
                <div
                  className={
                    filterState === filter[0].name
                      ? 'active-slider'
                      : 'active-slider position'
                  }></div>
              </div>
            </div>
            <div className='filter-container'>{buttons}</div>
          </div>
        </div>
      </div>
    </div>
